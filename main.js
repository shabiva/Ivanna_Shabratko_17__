...
class User {
    ...
    render() {
        ...
        <div class="user__info">
		    <div class="user__info--data">
        	    <img src="assets/images/users/${this.img}.png" alt="Jack Smith" height="50">
            ...
            <div class="user__info--role student">
        	    <img src="assets/images/roles/${this.role}.png" alt="student" height="25">
        	    <p>${this.role}</p>
            </div>
        </div>
    ...
    renderCourses() {
        return this.courses.length > 0 ? this.courses.map(({title, mark}) => `
		    <p class="user__courses--course ${this.role}">${title} <span class="${this.renderMark(mark)}">${this.renderMark(mark)}</span></p>
		`).join('') : '';
    }
